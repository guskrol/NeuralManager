param(
    ...
    [string]$ConfigFile = ".\data\farm.json",
    [int]$MaxInstances = 0,
...
function Read-FarmConfig {
    if (-not (Test-Path -LiteralPath $ConfigFile)) {
        throw "Config file not found: $ConfigFile. Copy farm.example.json to data\farm.json first."
    }
...
$accountsFile = [string](Get-OptionalValue -Object $config -Name "accountsFile" -DefaultValue ".\data\accounts.txt")
